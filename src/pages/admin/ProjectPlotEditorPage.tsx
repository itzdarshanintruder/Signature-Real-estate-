...
import { useCreateProjectPlot, useProjectPlot, useUpdateProjectPlot } from '@/hooks/use-project-plots'
import type { ProjectPlotImage } from '@/services/admin/projectPlotService'
...
export default function ProjectPlotEditorPage() {
  ...
  const { data: plot, isLoading } = useProjectPlot(id)
  const createMutation = useCreateProjectPlot()
  const updateMutation = useUpdateProjectPlot(id!, projectId!)

  const [images, setImages] = useState<ProjectPlotImage[]>([])
  ...
  return (
    <>
      <Seo title={`${isEditing ? 'Edit' : 'Add'} Plot | ${project?.title || 'Project'}`} />
      <AdminHeader />

      <header className="border-b border-ink-200 bg-cream-50 pt-10 pb-8">
        <Container>
          <Link
            to={`/admin/projects/${projectId}/plots`}
          ...
          >
            ...
            Back to {project?.title || 'Project'} Plots
          </Link>
              ...
              <div className="space-y-6 rounded-sm border border-ink-200 bg-cream-50 p-6 md:p-8">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Corner plot near park"
                  />
                </div>
              </div>
